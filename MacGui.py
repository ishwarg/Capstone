        ...
        if hasattr(self, 'csv_file_path'):
            with open(self.csv_file_path, newline='', encoding='utf-8-sig') as csvfile:
                ...
                for i, row in enumerate(csv_reader):
                    if i == 0:
                        self.initial_concentration = [float(val) for val in row]
                    elif i == 1:
                        self.decay_constant = [float(val) for val in row]
                    elif i == 2:
                        self.num_rows = int(row[0])
                    else:
                        self.data_points.append([float(val) for val in row])
            print("CSV file parsed successfully")
            print(self.initial_concentration)
